  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      const delayedTasks = await taskRepository.find({
        ...
        relations: ['assignedUser'] // Assuming 'assignedUser' is a relation in Task entity
      });
    ...
    scheduled: true,
    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'Asia/Kolkata' // Adjust timezone as per your requirements
  });
